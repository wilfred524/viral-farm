...
| **Render** | **Remotion se elimina.** Hook, chapa `n/N` y karaoke pasan a **ASS/libass** generado desde Python y quemado con ffmpeg. Saca Node del repo y baja el render de ~18 min por episodio a una estimación de 1-3 min (por validar). El paso queda tras una interfaz por si hiciera falta un motor de composición | 2026-08-05 |
